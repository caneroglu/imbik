...
2. Observe Ch2 output waveform on the oscilloscope:
   - **Predicted Asymmetry Index**: 0.000 (Look for positive vs negative peak clipping differences).
   - **Predicted Harmonics Ratio**: H2=0.000, H3=0.000, H5=0.000
...
This circuit is configured as an active filter with expected cutoff near **1 Hz**.

1. Set generator amplitude to **1.0 Vpp**.
2. Sweep generator frequency from 100 Hz up to 20 kHz.
3. Locate the -3dB frequency ($V_{out} = 0.707 \times V_{passband}$): Compare with simulated 1 Hz.
